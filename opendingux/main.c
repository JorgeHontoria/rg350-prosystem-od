...
#define tchepres(a) \
	keyboard_data[GameConf.Dingoo_Joy[a]] = 1;
...
{
#ifdef _VIDOD32_
	unsigned int *buffer_scr = video->pixels;
#else
...
int32_t joynumbuttons = 0;
int32_t joynumhats = 0;
int32_t joyaxis = 0;

static SDL_Joystick *joydev = NULL;


void initJoySDL(void)
{
	if (!SDL_InitSubSystem(SDL_INIT_JOYSTICK))
	{
		...
		if (joydev)
		{
			...
			joynumbuttons = SDL_JoystickNumButtons(joydev);
			joynumhats = SDL_JoystickNumHats(joydev);
			fprintf(stderr, "Joystick 1 has %d axes, %d buttons, and %d hat(s).\n", joynumaxes, joynumbuttons, joynumhats);
		}
	}
}
...
int main(int argc, char *argv[])
{
	...
	snprintf(home_path, sizeof(home_path), "%s/.rg350-prosystem-od", getenv("HOME"));

	snprintf(bios_filepath, sizeof(bios_filepath), "%s/7800.rom", current_conf_app);

	mkdir(current_conf_app, 0777);
	snprintf(current_conf_app, sizeof(current_conf_app), "%s/.rg350-prosystem-od/rg350-prosystem-od.cfg", getenv("HOME"));
	...
	GameConf.m_ScreenRatio = 1;

	SDL_WM_SetCaption("prosystem", NULL);
	...
	while (m_Flag != GF_GAMEQUIT)
	{
		...
		case GF_GAMEINIT:
			...
			if (cartridge_Load(gameName) != 0)
			{
				...
				// Init timing
				double period = 1.0 / (double)prosystem_frequency;
